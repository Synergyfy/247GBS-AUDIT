Based on **all the audit discussions** we've had over the past year, Henry's previous audit documents, the recent meeting transcript, and the earlier discussions around Excess Stock, Spare Capacity, Recommended Solutions, Agents, Account Managers, AI recommendations, and the MCOM ecosystem, the current audit at **247-gbs-audit.vercel.app** is only a small portion of what Henry envisions.

Henry is not building a questionnaire.

He is building a **Business Diagnostic & Solution Recommendation Platform**.

The audit itself is simply the mechanism that powers the business consultancy process.

---

# The Complete Audit Flow

Henry has repeatedly explained that the Audit has one purpose:

> Find problems → Prioritize them → Recommend the right solution → Assign the right implementation path.

Everything else supports that.

The flow should look like this:

```
Landing Page

↓

Business Understanding

↓

Free Triage

↓

Triage Analysis

↓

Audit Recommendation

↓

Choose Audit Path

↓

Business Information

↓

Sector Selection

↓

Sector Specific Audit

↓

Stage Progress

↓

Business Health Analysis

↓

Problem Classification

↓

Budget Assessment

↓

Time To Results Assessment

↓

Recommended Solutions Engine

↓

Prioritized Roadmap

↓

Implementation Packages

↓

AI Summary

↓

Account Manager / Agent Review

↓

Proposal

↓

Customer Accepts

↓

Projects Begin

↓

Monitoring Dashboard

↓

Periodic Re-Audit
```

This is the complete system Henry has been describing.

---

# PHASE 1

## Landing Page

The landing page is **not** simply "Start Audit."

The page must explain:

> Discover hidden profit.

> Find excess stock.

> Find spare capacity.

> Reduce unnecessary costs.

> Increase customers.

> Increase profit.

> Build a business growth plan.

Large CTA

```
Start Free Business Assessment
```

Henry repeatedly calls this

> Triage

or

> Free Audit

The wording can vary.

---

# PHASE 2

## Business Understanding

Before asking questions...

The customer should understand WHY they are doing the audit.

Henry repeatedly said:

People don't understand

what they're doing

or

why they're doing it.

Therefore introduce:

"This assessment will identify opportunities within your business and produce a prioritized action plan."

Only then begin.

---

# PHASE 3

## Business Registration

If user isn't logged in

Create account.

Business Name

Email

Phone

Business Address

Website

Sector

Employees

Years Trading

Annual Revenue (optional)

This becomes the Business Profile.

---

# PHASE 4

# FREE TRIAGE

Henry repeatedly calls this

Free Survey

Free Questions

Triage

Pre Audit

This is NOT the audit.

It simply determines

what audit they need.

Questions might include

Do you currently have excess stock?

Do you have unused staff capacity?

Are sales declining?

Is customer retention poor?

Do you run marketing campaigns?

Do you know your conversion rates?

Do you know your profit margin?

Do you know your customer lifetime value?

Simple questions.

Very quick.

5-10 minutes.

---

# PHASE 5

## TRIAGE ANALYSIS

Immediately after triage

System evaluates.

Decision:

```
Short Audit

OR

Long Audit
```

Customer never manually decides.

Henry questioned why users are choosing between short and long audits. The system should determine the appropriate path based on triage responses. 

---

# PHASE 6

## Explain Audit Recommendation

Instead of immediately starting.

Tell customer

Based on your answers...

We recommend

Long Audit

because

• Multiple operational issues detected

• Marketing weaknesses identified

• Inventory concerns found

Estimated Time

30 minutes

Start Audit

---

# PHASE 7

## Business Context

Before sector questions.

Collect:

Business Goals

Current Challenges

Biggest Problem

Current Marketing

Existing Software

Existing Loyalty System

Existing POS

Staff Count

Multiple Locations?

Everything needed to personalise recommendations.

---

# PHASE 8

# Sector Selection

Henry repeatedly asked

Where are the sectors?

Each business type has its own audit.

Restaurant

Salon

Retail

Dentist

Law Firm

Builder

Gym

Automotive

Manufacturer

Hotel

etc.

Once selected

Entire audit changes.

---

# PHASE 9

# Sector Specific Audit

Now the real audit begins.

This is where Henry said

the original audit

doesn't exist anymore.

Each sector has

its own questions.

Restaurant

Food Waste

Reservations

Table Turnover

Kitchen Efficiency

Delivery

Menu Engineering

Staff

Loyalty

Retail

Dead Stock

Fast Moving Stock

Inventory Turnover

Supplier Issues

Display

Footfall

Margins

Different questions.

---

# PHASE 10

## Stage Based Navigation

Henry specifically requested

NOT

Percentage

Instead

```
Stage 1 of 10

Business Overview

Stage 2

Stock

Stage 3

Customers

Stage 4

Marketing

Stage 5

Finance

Stage 6

Operations

Stage 7

Technology

Stage 8

Staff

Stage 9

Growth

Stage 10

Review
```

Every stage

shows

Questions Remaining

Questions Completed

Progress

---

# PHASE 11

## Smart Questions

Questions must not simply be

Yes

No

Henry has already rewritten several questions because they didn't capture enough information. 

Questions should gather:

Severity

Frequency

Scale

Percentage

Amount

Priority

Examples:

Instead of

Do you have excess stock?

Ask

How much excess stock?

0%

10%

20%

40%

60%

80%

100%

---

# PHASE 12

## AI Analysis

Behind the scenes

Every answer updates

Business Health

Inventory Score

Marketing Score

Sales Score

Visibility Score

Technology Score

Customer Score

Operations Score

etc.

No recommendations yet.

Just analysis.

---

# PHASE 13

## Business Health Dashboard

Customer now sees

Overall Business Health

Visual gauges

Radar charts

Traffic lights

Top Issues

Biggest Opportunities

Savings Potential

Growth Potential

---

# PHASE 14

## Budget Assessment

Henry repeatedly stated

Budget

is missing.

Now ask

Monthly Budget

Investment Budget

Maximum Spend

Preferred Payment

Internal Team Available?

Without this

Recommendations are incomplete.

---

# PHASE 15

## Time To Results

Henry repeatedly said

People want results yesterday.

Ask

Need Results

30 Days

60 Days

90 Days

6 Months

1 Year

System uses this

to prioritize.

---

# PHASE 16

# Recommended Solutions Engine

This is the heart of Henry's system.

Not generic AI advice.

Proper

Sector Specific

Business Specific

Budget Specific

Time Specific

Recommendations.

Each recommendation includes:

Problem

Evidence

Priority

Cost

Expected Outcome

Implementation Time

ROI

Dependencies

---

# PHASE 17

## Prioritization

Henry wants phased implementation.

Example

Phase 1

Fix dead stock

Phase 2

Install Loyalty

Phase 3

Customer Acquisition

Phase 4

Automation

Phase 5

Marketplace

Everything ordered by priority.

---

# PHASE 18

## Solution Packages

Now match

Recommended Solution

to

247GBS Services

Example

Dead Stock

↓

Excess Stock Program

Poor Customer Retention

↓

Rewards & Loyalty

Poor Visibility

↓

MCOM Mall

Unused Capacity

↓

Spare Capacity Program

Poor Marketing

↓

Campaign Management

Each recommendation links directly to the relevant MCOM solution.

---

# PHASE 19

## AI Executive Summary

Professional report.

Business Overview

Top Problems

Top Opportunities

Estimated Revenue Increase

Estimated Savings

Recommended Priority Plan

Budget Alignment

Timeline

---

# PHASE 20

## Agent / Account Manager Review

Henry repeatedly explained that this is where the Account Manager takes over.

The Account Manager reviews:

Audit

Solutions

Budget

Timeline

Then discusses the findings with the business owner:

> "This is how we're going to save you money. These issues have come out of the audit." 

The system should support internal notes, adjustments, and proposal preparation before presenting the final plan.

---

# PHASE 21

## Proposal

Generate

Professional Proposal

Implementation Plan

Pricing

Timeline

Deliverables

ROI

---

# PHASE 22

## Customer Acceptance

Accept Proposal

Book Meeting

Request Changes

Purchase Package

---

# PHASE 23

## Implementation Dashboard

Customer sees

Projects

Progress

Tasks

Milestones

Campaigns

Reports

Deliverables

---

# PHASE 24

## Continuous Monitoring

Henry has emphasized that the audit is not a one-off exercise. Once work begins, the business should be monitored and reassessed over time.

The dashboard should therefore include:

* KPI tracking
* Progress against recommendations
* Completed actions
* Outstanding issues
* Measured improvements

---

# PHASE 25

## Scheduled Re-Audit

Rather than ending with the first report, the platform should prompt businesses to perform periodic re-audits (for example every 3, 6, or 12 months, depending on their programme).

Each re-audit compares:

* Previous audit vs. current audit
* Problems resolved
* New issues detected
* Improvement trends
* Remaining opportunities

This reinforces Henry's concept that the audit is an ongoing business improvement process rather than a single questionnaire.

---

## How this differs from the current implementation

From Henry's comments and the current prototype, the main gaps are:

* The current flow does not clearly separate **Triage** from the main audit.
* Users should not manually choose Short vs. Long Audit; the system should decide after triage.
* Sector-specific question sets need to replace generic questions.
* The audit should be organized into clearly defined stages rather than only showing percentage completion.
* Questions need to capture actionable business data, not just yes/no responses.
* Budget and desired time-to-results must be collected before recommendations.
* The recommendation engine must produce tailored, business-specific implementation plans instead of generic advice.
* The audit must transition directly into the Account Manager and proposal workflow.
* The platform should support continuous monitoring and scheduled re-audits, making the audit the foundation of an ongoing consultancy relationship rather than a standalone form.

This end-to-end flow aligns with Henry's repeated description of the audit as a **business diagnostic, prioritization, and solution delivery system**, not merely an assessment form.

---

# Public Pre-Audit (current implementation)

The **Public Pre-Audit** is the front-door implementation of PHASE 4/5/6 above. It is a fully public, no-account required, data-driven question engine built on top of the existing Business Triage content.

- **Routes** (web): `/audit/pre-audit` (public landing) and `/audit/pre-audit/flow` (the engine).
- **Question data**: consumed live from the public endpoints `GET /api/v1/triage/questions/start` and `GET /api/v1/triage/questions/:id`. No question content is hardcoded in the frontend.
- **Flow**: loading → question → email → review → submitting → confirmation (plus error states). Back navigation and edit-from-review **recalculate the active branch path** and drop answers that are no longer reachable.
- **Progress**: derived from the active path only (`Question N`, `N answered`). No hardcoded "Step X of 20".
- **Persistence**: progress and the completed result (email + answers + recommended audit + fingerprint) are stored in `localStorage` (`247gbs_preaudit_progress`, `247gbs_preaudit_result`, `247gbs_preaudit_submissions`). Duplicate submissions (same email + same answers) are detected via a fingerprint and prevented.
- **Fetching**: uses the same direct-`fetch` helpers as the existing triage page (`services/triage/flow`). The shared `apiClient` 401-redirect is intentionally **not** used so anonymous users never see a bogus "session expired" error.

## Backend gaps (explicitly out of scope for the frontend-only build)

These are the deliberate boundaries of the current implementation. Address them server-side before treating the Pre-Audit as production-end-to-end:

1. **No public submission endpoint.** `POST /api/v1/triage` requires a JWT and a fixed DTO. There is no public endpoint to store a pre-audit + email on the server, so submissions currently live only in the visitor's browser. A public endpoint (e.g. `POST /api/v1/pre-audit` storing email + answers) is the natural next step.
2. **Question model has no `type`.** `TriageQuestion` only exposes single-choice answers (`nextQuestionId` / `auditType`). There is no `type` column for multi-select, text, number, or email questions. The Pre-Audit engine is forward-compatible: it honours a question `type` of `multi-select` if one is ever added, and otherwise defaults to single-choice.
3. **Full Audit handoff is user-bound.** `AuditSession` requires a `userId` and is created via the authenticated `POST /triage`. A public pre-audit therefore cannot create a server-side audit session. The confirmation currently links to `/audit/flow?type=SHORT_FORM|LONG_FORM`, which runs the full audit UI but, for anonymous visitors, still has no persisted server record until they create an account and complete the authenticated triage.
4. **Email is not persisted server-side**, so result emails cannot yet be sent from a public submission.
5. **Admin triage endpoints are `@Public()`** (`/api/v1/admin/triage/*`) — a pre-existing anomaly. They are not guarded and should be secured before production.

## Verified data properties (checked against a live local API)

- The seeded triage graph is acyclic (no infinite loops).
- Every branch terminates with a `SHORT_FORM` or `LONG_FORM` recommendation, so the Pre-Audit reliably reaches the email → review → confirmation steps.
- Maximum path depth is 4 questions.