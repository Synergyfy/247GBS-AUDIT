...
export function useIntelligence() {
  ...
  const fetchData = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      let res = await fetch(`${API_BASE_URL}/dashboard/intelligence`, { method: "GET", headers, signal });

      if (res.status === 401) {
        const newToken = await refreshAccessToken();
        if (newToken) {
          headers["Authorization"] = `Bearer ${newToken}`;
          res = await fetch(`${API_BASE_URL}/dashboard/intelligence`, { method: "GET", headers, signal });
        }
      }
