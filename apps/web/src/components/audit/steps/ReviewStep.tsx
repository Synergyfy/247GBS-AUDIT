...
import { ArrowRight, CheckCircle2, Building2, Package, Users, ShieldCheck, Loader2 } from "lucide-react";
    ...
    const onConfirm = async () => {
        ...
        try {
            ...
            let res = await fetch(`${API_BASE_URL}/audit`, {
                ...
                body: JSON.stringify({
                    auditType: "LONG_FORM",
                    scopes: ["excessStock", "capacityServices"],
                }),
            });
            ...
            if (!res.ok) {
                ...
                throw new Error(errJson.message || `Failed to create audit session (${res.status})`);
            }

            const session = await res.json();
            const answers: Record<string, any> = {};
            if (data.excessStock?.length) answers.excessStock = data.excessStock;
            if (data.capacityServices?.length) answers.capacityServices = data.capacityServices;
            if (data.basics) answers.basics = data.basics;
            if (data.costs) answers.costs = data.costs;
            if (data.rules) answers.rules = data.rules;

            const putRes = await fetch(`${API_BASE_URL}/audit/${session.id}/answers`, {
                method: "PUT",
                headers,
            ...
            if (!putRes.ok) {
                ...
                throw new Error(errJson.message || `Failed to submit answers (${putRes.status})`);
            }
        ...
        >
            <Card className="w-full max-w-2xl glass-card border-orange-100 max-h-[80vh] flex flex-col">
                ...
                <CardContent className="space-y-6 overflow-y-auto flex-1 pr-2 custom-scrollbar">
                    ...
                    <Button onClick={onConfirm} className="w-full mt-4" size="lg" variant="gradient" disabled={submitting}>
                        {submitting ? (
                            <>
